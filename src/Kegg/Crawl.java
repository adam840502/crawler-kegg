...
public class Crawl {
	...
	public static List<Organ> search(String keyword){
		List<Organ> match = new ArrayList<>();
		for(Organ organ : organList){
			boolean tNumMatch = organ.getTNumber().matches("(.|\n)*"+keyword+"(.|\n)*");
			boolean sciMatch = organ.getScientificName().matches("(.|\n)*"+keyword+"(.|\n)*");
			boolean keyMatch = false;
			for(String s : organ.keywords){
				if(s.matches("(.|\n)*"+keyword+"(.|\n)*")) keyMatch = true;
			}
