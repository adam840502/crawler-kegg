...
public class TypeTask implements Runnable {
    ...
    public void run() {
        ...
        if(this.organ==null){
            System.out.println("can't find org");
            return;
        }else {
        ...
        try {
            classListString = Jsoup.connect(Crawl.listUrl+type+"/"+organ.getOrg()).execute().body();
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            linkListString = Jsoup.connect(Crawl.linkUrl + organ.getOrg() + "/" + type).execute().body();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void getGeneNumbers(String linkListString){
        ...
        System.out.println("get gene numbers");
